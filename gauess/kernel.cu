...
__global__ void GaussianEliminationGlobal(const int clusterSize,float *x, const float *diagonal_values , const float *non_diagonal_values ,float *y , const int size)
{
	...
	//shared_m[0][0] = diagonal_values[0];
...
void gaussianCuda(const int noElem, char* file_name)
{
    ...
    GaussianEliminationGlobal<<<20, 1/*,(clusterSize * clusterSize) * sizeof(float)*/>>>(clusterSize,dev_x, dev_diagonal_values , dev_non_diagonal_values , dev_y,size);
	GaussianEliminationShared<<<20, 1/*,(clusterSize * clusterSize) * sizeof(float)*/>>>(clusterSize,dev_x, dev_diagonal_values , dev_non_diagonal_values , dev_y);
